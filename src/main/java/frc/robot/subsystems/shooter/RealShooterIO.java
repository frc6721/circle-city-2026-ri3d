...
public class RealShooterIO implements ShooterIO {
    ...
    public void configFlywheelMotor(){
        ...
        config.inverted(ShooterConstants.SHOOTER_FLYWHEEL_INVERTED)
        ...
        config.encoder
            .positionConversionFactor(2 * Math.PI / ShooterConstants.FLYWHEEL_GEAR_RATIO) // motor rotations to flywheel rad
            .velocityConversionFactor((2 * Math.PI) / 60.0 / ShooterConstants.FLYWHEEL_GEAR_RATIO)// motor RPM to flywheel rad/s
            .inverted(false);
        config.closedLoop
            .pidf(
        ...
        tryUntilOk(
            ...
            5,
            () ->
            _flywheelMotor.configure(config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters));
    }
    ...
    public void updateInputs(ShooterIOInputs inputs){
        ...
        inputs._flywheelMotorVoltage = Volts.of(_flywheelMotor.getAppliedOutput() * _flywheelMotor.getBusVoltage());
    ...
    public void setFlywheelSpeed(AngularVelocity speed) {
        _flywheelMotor.getClosedLoopController().setReference(speed.in(RadiansPerSecond), ControlType.kVelocity);
    }
