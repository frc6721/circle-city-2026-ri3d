...
public class RealIntakeIO implements IntakeIO {
  ...
  public void configPivotMotor() {
    ...
    config
        ...
        .positionConversionFactor(
            2
                * Math.PI
                / IntakeConstants.INTAKE_PIVOT_CONVERSION_FACTOR) // motor rotations to intake rad
        .velocityConversionFactor(
            (2 * Math.PI)
                / 60.0
                / IntakeConstants.INTAKE_PIVOT_CONVERSION_FACTOR) // motor RPM to intake rad/s
        .inverted(false);
  ...
  public void updateInputs(IntakeIOInputs inputs) {
    ...
    inputs._intakePivotMotorVoltage =
        Volts.of(_pivotMotor.getAppliedOutput() * _pivotMotor.getBusVoltage());
  ...
  public void zeroPivotEncoder() {
    _pivotMotor.getEncoder().setPosition(0.0);
  }
