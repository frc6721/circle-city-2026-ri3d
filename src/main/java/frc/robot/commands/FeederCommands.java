 ...
 *
 * <p>The feeder commands are simple since the feeder subsystem doesn't require complex control - it
 * either feeds game pieces, stops, or runs at a specific speed.
 *
 * <p><b>Typical Usage Pattern:</b> The feeder is usually controlled automatically as part of a
 * shooting sequence:
 *
 * <ol>
 *   <li>Shooter spins up to target RPM
 *   <li>Wait for shooter to reach speed
 *   <li>Run feeder to push game piece into shooter
 *   <li>Stop feeder after shot
 * </ol>
 *
...
public class FeederCommands {
   ...
   *
   * <p>This applies a deadband to the joystick input to prevent drift from causing unwanted
   * movement. Small joystick values near zero are ignored.
   *
   * <p><b>When to use:</b>
   *
   * <ul>
   *   <li>Manual testing during practice
   *   <li>Troubleshooting feed issues
   *   <li>Fine control during setup
   * </ul>
   *
   ...
   * </pre>
   *
   * <p><b>Deadband:</b> Values between -0.1 and +0.1 are treated as 0 to prevent drift when the
   * joystick isn't perfectly centered.
   *
   ...
   *
   * <p>This is the most common way to control the feeder - run at a constant speed for a set
   * duration. Typically used in shooting sequences.
   *
   * <p><b>Speed values:</b>
   *
   * <ul>
   *   <li>+1.0 = Full power forward (feed into shooter)
   *   <li>+0.7 = 70% power (typical for controlled feeding)
   *   <li>0.0 = Stopped
   *   <li>-0.5 = Half power reverse (clear jam or pull back)
   * </ul>
   *
   ...
   * </pre>
   *
   * <p><b>Tuning tip:</b> Start with lower speeds (0.5-0.7) and increase if needed. Too fast can
   * cause game pieces to bounce or jam.
   *
   * @param feeder The feeder subsystem
   ...
   *
   * <p>This command runs once to stop the feeder, then immediately finishes. Unlike the
   * continuously-running commands above, this uses runOnce().
   *
   * <p><b>When to use:</b>
   *
   * <ul>
   *   <li>At the end of shooting sequences
   *   <li>When aborting a shot
   *   <li>Emergency stop
   *   <li>After manually clearing a jam
   * </ul>
   *
   ...
   * </pre>
   *
   * <p><b>Note:</b> Using runOnce() means this command executes once and finishes immediately,
   * unlike run() which would continue indefinitely. This is appropriate for stopping because we
   * just need to send the stop command once.
   *
   * @param feeder The feeder subsystem
